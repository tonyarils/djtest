  ...
  <el-card>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="父级党组织">
          <el-select v-model="form.parent_id" placeholder="请选择父级党组织" style="width: 100%">
            <el-option v-for="option in parentSelectOptions" :key="option.id ?? 'top'" :label="option.name" :value="option.id" />
          </el-select>
        </el-form-item>
...
const syncParentId = () => {
  ...
  if (!parentSelectOptions.value.length) {
    form.value.parent_id = null
    return
  }
  const matched = parentSelectOptions.value.some((item) => item.id === form.value.parent_id)
  if (!matched) {
    form.value.parent_id = parentSelectOptions.value[0].id
  }
}
...
watch(() => form.value.parent_id, (newVal) => {
  if (!newVal) {
    form.value.level = 1
  }
  syncParentId()
